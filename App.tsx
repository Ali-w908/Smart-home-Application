...
export default function App() {
  ...
  // 1. Subscribe to Arduino Service (Real Hardware Communication)
  useEffect(() => {
    const unsubscribe = arduinoService.subscribe((state: ArduinoState) => {
      setLampOn(state.lampOn);
      setPlugOn(state.plugOn);
      setDoorStatus(state.doorStatus);
      setTemperature(state.temperature);
      setBuzzerOn(state.buzzerOn);
      setAlarmThreshold(state.alarmThreshold);
  ...
  // 3. Logic for Alarm Sync (Software -> Hardware) - Only send if connected
  useEffect(() => {
    if (!isConnected) return;

    // If our calculated alarm state differs from the buzzer state, sync
    if (isAlarmActive && !buzzerOn) {
      arduinoService.sendCommand({ type: 'ALARM_ON' });
    } else if (!isAlarmActive && buzzerOn) {
      arduinoService.sendCommand({ type: 'ALARM_OFF' });
    }
  }, [isAlarmActive, buzzerOn, isConnected]);

  // --- Handlers ---

  const handleToggleLamp = async (newState: boolean) => {
    setIsLoadingLamp(true);
    // Use LAMP_TOGGLE for XOR behavior - the Arduino will handle the XOR with hardware switch
    await arduinoService.sendCommand({ type: 'LAMP_TOGGLE' });
    setIsLoadingLamp(false);
  };

  const handleTogglePlug = async (newState: boolean) => {
    setIsLoadingPlug(true);
    await arduinoService.sendCommand({ type: newState ? 'PLUG_ON' : 'PLUG_OFF' });
  ...
  const handleSaveThreshold = async (newThreshold: number) => {
    ...
    if (isConnected) {
      await arduinoService.sendCommand({ type: 'SET_THRESHOLD', value: newThreshold });
    }
  };
  ...
  return (
    <div className={`min-h-screen pb-20 transition-colors duration-500 safe-bottom ${isAlarmActive ? 'bg-red-50' : 'bg-gray-50'}`}>

      {/* Header */}
      <header className={`sticky top-0 z-30 backdrop-blur-md bg-white/70 border-b border-gray-200 px-6 py-4 flex justify-between items-center transition-colors safe-top ${isAlarmActive ? 'bg-red-100/80 border-red-200' : ''}`}>
        <div>
